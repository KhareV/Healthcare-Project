...
def register_package(root, config_path, manifest_path, items):
    ...
    # Phase-22 is a generated current-state evidence package at fixed paths.
    # Replace only its own prior records atomically; retaining them alongside
    # regenerated bytes would create duplicate immutable paths and stale hashes.
    # Repository history preserves earlier package identities. All scientific
    # and other non-Phase-22 lineage records remain untouched.
    records = [
        record for record in read_artifact_index(index)
        if not record.artifact_id.startswith("phase22:")
    ]
    ...
    write_artifact_index(index, records)
