...
CanonicalFeatureBuilder = Callable[[object], CanonicalExample]
...
class CanonicalHistoryInputProvider:
    ...
    def __init__(
        ...
        *,
        ...
        self._last_example: Optional[CanonicalExample] = None
    ...
    def get_canonical_input(
        ...
        *,
        ...
        if example.stay_id != stay_id or example.prediction_time != prediction_time:
            raise ServingFeatureContractError("canonical builder returned the wrong example")
        if example.eligibility != _EMPTY_ELIGIBILITY or example.targets != _EMPTY_TARGETS:
            raise ServingFeatureContractError(
