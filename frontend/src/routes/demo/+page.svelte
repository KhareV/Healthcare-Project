<script lang="ts">
	...
	import { Play, Pause, RotateCcw, SkipForward, SkipBack, Maximize2, Minimize2, Activity, BrainCircuit, BellRing, Waypoints } from '@lucide/svelte';
...
<div class="demo-shell" class:danger={tone === 'danger'}>
	...
	{:else if subject && current}
		...
		<section class="metric-grid">
			<article><span>CURRENT SOFA</span><strong>{current.current_sofa.toFixed(1)}<small>/24</small></strong><footer>OBSERVED AT CUTOFF {cutoffs[cutoffIndex].slice(11, 16)}</footer></article>
			<article><span>PREDICTED +24H / +48H</span><strong>{current.recovery.sofa_hat_24h.toFixed(1)}<small>→ {current.recovery.sofa_hat_48h.toFixed(1)}</small></strong><footer>INDEPENDENT RECONSTRUCTIONS</footer></article>
			<article><span>REMAINING ICU STAY</span><strong>{current.icu_stay_time.remaining_hours.toFixed(1)}<small>h</small></strong><footer>POSTPROCESSED FORECAST</footer></article>
			<article class:alert={current.organ_support.alert}><span>SUPPORT RISK (24H)</span><strong class="state">{(current.organ_support.probability_24h * 100).toFixed(1)}<small>%</small></strong><footer>{current.organ_support.alert ? 'ABOVE THRESHOLD' : 'BELOW THRESHOLD'}</footer></article>
		</section>
		...
		<section class="pipeline" aria-label="System pipeline">
			...
			<div class:active={current.organ_support.alert}><BellRing size={17} /><span>04 · THRESHOLD</span><b>SUPPORT ALERT</b></div>
		</section>

		<section class="main-grid">
			<div class="trend">
				<div class="trend-card">
					<header><span>SOFA / REPLAY SO FAR</span><b>{subject.subject_id}</b></header>
					<MultiLine series={[{ name: 'Observed SOFA', color: '#2bb8b0', values: historySofa }]} xLabels={cutoffs.slice(0, cutoffIndex + 1).map((c) => c.slice(11, 16))} yMin={0} yMax={24} height={190} />
				</div>
			</div>
...
<style>
	...
	.main-grid{display:grid;grid-template-columns:minmax(0,1.65fr) minmax(280px,.55fr);gap:7px}.trend-card{padding:18px;border:1px solid rgba(148,163,184,.14);background:#070d18}.trend-card header{display:flex;justify-content:space-between;margin-bottom:12px;color:#53647b;font:7px 'JetBrains Mono',monospace;letter-spacing:.1em}.trend-card header b{color:#dce9e8}.reasoning{display:grid;gap:7px}.reasoning>div{padding:16px;border:1px solid rgba(148,163,184,.14);background:#080f1b}.decision{color:var(--accent)}.decision.blocked{color:#fb7185}.decision>span{display:inline-block;margin-bottom:8px;color:#64748b;font-size:7px;letter-spacing:.13em}.decision strong{display:block;margin:6px 0 9px;font:500 18px/1.1 'Space Grotesk',sans-serif}.decision p,.events p{margin:0;color:#71829a;font-size:10px;line-height:1.6}.events>span{display:block;margin-bottom:15px;color:#64748b;font-size:7px;letter-spacing:.13em}.events div{display:grid;grid-template-columns:8px 38px 1fr;align-items:center;gap:6px;margin:10px 0;font:7px 'JetBrains Mono',monospace}.events div i{width:5px;height:5px;border-radius:50%;background:#2bb8b0}.events div i.danger{background:#fb7185}.events div b{color:#53647b}.events div small{color:#91a8b8}
