<script lang="ts">
	...
	import TrajectoryCopilot from './TrajectoryCopilot.svelte';
	const NAV = [
		...
		{ label: 'DATA QUALITY', detail: 'Provenance', path: '/system/data', icon: DatabaseZap },
		{ label: 'DEMO', detail: 'Walkthrough', path: '/demo', icon: MonitorPlay }
	];
...
<div class:mobile-open={mobileOpen} class="dashboard-shell">
	...
	<aside class="sidebar" aria-label="Primary workspaces">
		<a class="brand" href="/" onclick={() => (mobileOpen = false)} aria-label="Personalized Patient Recovery Trajectory home"><strong>PRT · V2</strong></a>
