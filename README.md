...
- **My Health Record** (`/health-record`), an optional persistent longitudinal layer (`src/serving/v2/persistence.py`, MongoDB): a per-user conditions list, and every custom record's raw structured input plus a prediction-run history across the cutoffs actually replayed. When `MONGODB_URI` is configured, custom records survive an API restart (transparently rehydrated into the in-memory serving runtime from the raw input on first access); without it, behavior is unchanged from the original design (in-memory only, discarded on restart). MongoDB is never read by the scientific pipeline itself — it only ever supplies the same raw input the direct-entry form always collected.
